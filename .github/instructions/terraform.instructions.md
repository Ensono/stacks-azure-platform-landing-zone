...
- Use Azure environment variables to reference values stored in Azure Key Vault.
  - This keeps sensitive values out of your Terraform state files.
- Never commit sensitive information such as Azure credentials, API keys, passwords, certificates, or Terraform state to version control.
...
- Use Azure RBAC (Role-Based Access Control) and managed identities to control access to resources.
  - Follow the principle of least privilege when assigning permissions.
- Use Network Security Groups (NSGs) and Azure Firewall to control network access to resources.
- Deploy resources in private subnets whenever possible.
  - Use public subnets only for resources that require direct internet access, such as Application Gateways or Azure Firewall.
- Use encryption for sensitive data at rest and in transit.
  - Enable encryption for managed disks, Storage Accounts, and Azure SQL databases.
...
- Group related resources together in the same file.
  - Use a consistent naming convention for resource groups (e.g., `providers.tf`, `variables.tf`, `network.tf`, `app_service.tf`, `sql_database.tf`).
