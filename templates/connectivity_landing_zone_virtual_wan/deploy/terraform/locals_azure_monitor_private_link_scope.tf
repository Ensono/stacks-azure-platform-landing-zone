locals {
  ...
  # Get DNS zone IDs from the virtual_wan module output (primary hub has all zones)
  ampls_dns_zone_ids = var.azure_monitor_private_link.enabled ? [
    for key in local.ampls_required_dns_zone_keys :
    module.virtual_wan.private_dns_zone_resource_ids[local.primary_hub_region][key]
  ] : []
}
