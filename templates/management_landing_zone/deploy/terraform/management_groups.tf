locals {
  ...
  # Note: We use azapi_client_config (not azurerm_client_config) to ensure static/known values
...
# Validate required subscriptions when management groups are enabled (skip if skip_subscription_placement)
